  ...
  check = ({ programID, linePosition }: CheckParams) => {
    ...
    if (!varName)
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: 'Se hace una declaracion "divida" pero no se especifica la variable requerida',
      })
    ...
    if (!variable)
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: `La variable ${varName} indicada en la declaracion de division, no existe en el programa`,
      })
    ...
    if (
      ...
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: `La variable ${varName} indicada en la declaracion de division no es de un tipo valido para realizar la operacion`,
      })
