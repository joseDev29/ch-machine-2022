...
@Injectable()
export class RuleExponentiation {
  ...
  check = ({ programID, linePosition }: CheckParams) => {
    ...
    if (!varName)
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: 'Se hace una declaracion "potencia" pero no se especifica la variable requerida',
      })
    ...
    if (!variable)
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: `La variable ${varName} indicada en la declaracion de potencia, no existe en el programa`,
      })
    ...
    if (
      ...
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: `La variable ${varName} indicada en la declaracion de potencia no es de un tipo valido para realizar la operacion`,
      })
    ...
    program.memoryBlock.push({
      ...
      varName,
      lineText: `${line.join(' ')}`,
      lineType: LINE_TYPE.exponentiation,
    })
