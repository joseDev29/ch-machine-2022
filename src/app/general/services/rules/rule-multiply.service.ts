  ...
  check = ({ programID, linePosition }: CheckParams) => {
    ...
    if (!variable)
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: `La variable ${varName} indicada en la declaracion de multiplicacion, no existe en el programa`,
      })
    ...
    if (
      ...
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: `La variable ${varName} indicada en la declaracion de multiplicaion no es de un tipo valido para realizar la operacion`,
      })
