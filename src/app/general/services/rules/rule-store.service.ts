  ...
  check = ({ programID, linePosition }: CheckParams) => {
    ...
    if (!varName)
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: 'Se hace una declaracion "almacene" pero no se especifica la variable requerida',
      })
    ...
    if (!program.variables.get(varName))
      return this.machineState.codeErrors.push({
        programID,
        line: linePosition + 1,
        text: `La variable ${varName} indicada en la declaracion de almacenar, no existe en el programa`,
      })
